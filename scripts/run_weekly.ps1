# run_weekly.ps1 - Paper_Cross_Weekly 用 (UTF-8 BOM + CRLF / 日本語パス対応)
...
# 2) publish --weekly (claude -p で週次レポも生成)
Log "--- publish.py --weekly ---"
& $PY "$PJ\publish.py" --weekly *>&1 | ForEach-Object { Add-Content -LiteralPath $LOG -Value $_ -Encoding UTF8 }
...
if ($hasChanges) {
    $msg = "auto: weekly report $(Get-Date -Format 'yyyy/MM/dd')"
    Log "--- git commit ---"
